...
export class PickList {
	...
	public static createItemLIst() {
		...
		let items: ImgItem[] = [
			...
			{
				label: '$(heart)   Setup switching time      ',
				description: '设置切换背景时长',
				imageType: 12
			},
		];
	...
	// 列表点击事件分配
	private listChange(type: number, path?: string | string[]) {
		switch (type) {
			case 1:
				this.imgList();  // 展示图片列表
				break;
		...
		if (this.checkFolder(this.config.randomImageFolder)) {
			...
			if (files.length > 0) {
				...
				files = files.map(file => {
					return path.join(this.config.randomImageFolder, file);
				}).sort(() => Math.random() > 0.5 ? -1 : 1);
				// let randomFile = files[Math.floor(Math.random() * files.length)];
				// let file = path.join(this.config.randomImageFolder, randomFile);
				this.listChange(4, files);
			}
		}
		return true;
	}
	...
	private showInputBox(type: number) {
		...
		switch(type) {
			case 1:
				...
				promptString = '请输入图片路径，支持本地及https';
				break;
			case 2:
				placeString = 'Opacity ranges：0.00 - 1,current:(' + this.opacity + ')';
				promptString = '设置图片不透明度：0-1';
				break;
			case 3:
				placeString = 'Setup switching time (20 - 100000),current:(' + this.duration + ')';
				promptString = '设置切换背景时长：20-100000';
				break;
		}
		...
		vscode.window.showInputBox(option).then(value => {
			...
			if (type === 1) {
				...
				let isDuration = parseInt(value);

				if (isDuration < 20 || isDuration > 100000 || isNaN(isDuration)) {
					vscode.window.showWarningMessage('Duration ranges in：20 - 100000！');
					return false;
				}
			}
			...
			this.setConfigValue(configName, configValue, true);
		})
	}

	// 更新配置
	private updateBackgound(path?: string | string[]) {
		if (!path) {
	...
	private updateDom(uninstall: boolean = false) {
		let dom: FileDom = new FileDom(this.imgPath, this.opacity, this.duration);
		let result = false;
		if (uninstall) {
